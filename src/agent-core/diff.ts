...
// Head of a block — the opener marker through the `:start_line:` token — kept in ONE source string shared by SEARCH_RE and countPlausibleSearchOpeners. The two MUST accept the same whitespace (`\s*` spans blank lines): a head that parseDiffBlocks consumes but the counter doesn't see breaks the counter's "can never parse MORE blocks than this" invariant, and the malformed-block guard then waves a merged garbage block through.
...
/**
 * Guard against `parseDiffBlocks` silently mangling a malformed diff. Two distinct failure shapes are checked, and `null` means the diff is sound:
 *
 * - an opener the regex never consumed (e.g. a block missing its `:start_line:` line) — that block would be silently skipped while the rest applied "successfully";
 * - an opener ABSORBED into a neighbouring block's span (a block missing its `=======` or `>>>>>>> REPLACE` makes the lazy regex run through the next block), which would apply a merged garbage block — writing literal diff marker lines into the file — and report success.
 *
 ...
 * (editing a file that embeds diff markers). Payload that reproduces the full opener+`:start_line:` sequence still trips the absorbed-count — erring toward a loud false reject over silent corruption.
 *
 * Shared by `applyDiff` and the per-host executors that run their own apply loop, so the diagnostics cannot drift between clients.
 */
export function malformedDiffBlocksError(
  ...
  // The two counts can flag the SAME block (an unconsumed opener that also carries `:start_line:`), so report the larger, not the sum.
...
export function locateBlock(text: string, block: DiffBlock): BlockMatch {
  ...
  // Resolve the match by the exact occurrence NEAREST to the :start_line: hint, not the first one an anchored scan happens to reach. When the SEARCH text is unique this is trivially that single copy; when it repeats, nearest-to-hint is what stops the edit from landing on the wrong duplicate — including a duplicate sitting just BEFORE the hint but inside the anchor window, which a first-match-after-anchor search would otherwise pick over the real target.
  ...
  if (offsets.length > 0) {
    ...
    if (bestDistance <= DIFF_BUFFER_LINES) {
      // A distance-0 hit names the intended copy unambiguously (even if the SEARCH text also occurs elsewhere), so it stays warning-free like any clean match. Flag ambiguity only when duplicates exist AND the hint didn't land exactly on one — surfacing which copy proximity picked.
    ...
    if (offsets.length <= 1) {
      // Unambiguous: this is the only place the SEARCH text exists at all, just not where :start_line: said (e.g. a stale hint from an earlier block's line-delta estimate in the same diff). It's still the correct edit target — accept it, but flag the mismatch rather than pretending the hint was accurate.
      return {
        found: true,
        index: best,
        length: block.search.length,
    ...
    // Ambiguous, and none of the copies are near the hint: don't guess which one is intended. Fall through to fuzzy matching for a genuinely different near-hint region (e.g. the real target was edited slightly and no longer matches exactly) — but if fuzzy only rediscovers one of these same exact duplicates, report the ambiguity instead of the generic distance warning below.
  }
  ...
  if (fz && fz.score >= FUZZY_THRESHOLD) {
    ...
    if (distance <= DIFF_BUFFER_LINES) {
      // The SEARCH text (ignoring trailing whitespace) matched more than one window; bestFuzzyWindow picked the copy nearest the hint. Surface which one — mirroring the exact-match path — unless the hint landed exactly on a copy (distance 0), which is unambiguous and stays warning-free.
    ...
    if (offsets.length > 1) {
      // The fuzzy whole-file fallback just rediscovered one of the ambiguous exact duplicates ruled out above (identical text always scores a perfect 1.0) — don't silently accept it as if it were a confident fuzzy match; report the same ambiguity as a miss instead.
      return {
        found: false,
        index: -1,
        length: 0,
  ...
  if (offsets.length > 0) {
    // Exact matches exist, but none of them (nor the fuzzy fallback) landed near the hint — report this as a miss with an explicit ambiguity count instead of silently applying an arbitrary copy.
    return {
      found: false,
      index: -1,
      length: 0,
...
export function bestFuzzyWindow(
  ...
  // File-wide count of windows trim-equal to the SEARCH (cheap — no Levenshtein), so the duplicate-ambiguity warning reflects the WHOLE file and not merely whichever sub-range the returned window came from. Without this, a SEARCH duplicated more than DIFF_BUFFER_LINES from the hint would be applied to the near copy with no warning, purely because the far copy fell outside the near scan's range.
  ...
  const scan = (from: number, to: number): FuzzyWindow | null => {
    ...
    // Once any trim-equal (score-1) window is seen, no other window can beat it, so stop paying for similarity()/Levenshtein and only keep scanning the cheap trim-equal windows for the nearest-to-hint tiebreak. This restores the early-out the old `score >= 0.99` return gave, without reintroducing the first-in-iteration-order duplicate bug.
    ...
    for (let i = Math.max(0, from); i <= last; i++) {
      ...
      // Prefer the higher score; among equally-scored windows keep the one NEAREST the start_line hint, so a SEARCH block trim-equal (or ~identical) to several windows edits the copy the caller pointed at rather than the first in iteration order — mirroring the exact-match path above.
...
export function applyDiff(
  ...
  // Guard against parseDiffBlocks silently mangling a malformed diff (dropped or absorbed blocks) — see malformedDiffBlocksError for the failure shapes.
  ...
  for (let bi = 0; bi < blocks.length; bi++) {
    ...
    if (opts.replaceAll && raw.search !== '') {
      // Replace EVERY exact occurrence of the SEARCH block (CRLF/LF and trailing-whitespace tolerance applied per occurrence). The line anchor is irrelevant here, so don't shift the delta into it.
