 ...
 */
// Inner-quantifier levels tracked per group depth. The distinction matters
// under a BOUNDED-variable outer interval: a bounded inner ((a{1,3}){2,4})
// caps the ambiguity at (width choices)^m — constant in input length — while
// an unbounded inner ((a*){2,8}) lets the path count grow with the input
// (~L^m), so only the latter is evil under a small bounded outer.
const INNER_NONE = 0
...
export function hasNestedQuantifier(source: string): boolean {
  ...
  for (let i = 0; i < source.length; i++) {
    ...
    if (c === ')') {
      ...
      // How evil the outer quantifier is depends on both sides (uses the same
      // quantifierAt parse as the alternation check so both stay consistent):
      // - unbounded outer (`+`, `*`, `{n,}`): catastrophic over ANY variable
      //   inner — `(a+)+`, `(a{1,3})+` all have input-length-many split points;
      // - bounded-variable outer `{n,m}`: with an UNBOUNDED inner the path
      //   count still grows with the input (`(a*){2,8}` ~ L^8); with a bounded
    ...
    if (depth > 0) {
      if (c === '+' || c === '*') {
        ...
        // `{n,}` (open-ended, unbounded) and `{n,m}` with m > n (bounded)
        // both let each iteration of the enclosing group consume a different
        // amount. Only a fixed-count `{n}` (or degenerate `{n,n}`) expands to
        // a linear string and counts for nothing (`(a{3})+` is safe), and a
        // `{` that is not a valid interval is a literal.
 ...
 * flagged; an unquantified/fixed-count group (`(a|a)`, `(a|a){2}`) is safe and
 * ignored, as is a small bounded repeat (`(a|a){2,4}` — at most
 * SMALL_BOUNDED_REPEAT_MAX ambiguous iterations, a constant a backtracking
 * engine shrugs off). Like hasNestedQuantifier, a cheap heuristic: it does not
 * chase overlap hidden inside a character class or across multi-atom branches
 * (`(ab|a[bc])+`).
 */
export function hasOverlappingAlternation(source: string): boolean {
  ...
  for (let i = 0; i < source.length; i++) {
    ...
    if (c === ')') {
      ...
      const q = quantifierAt(source, i + 1)
      if (!q) continue
      // Fixed-count repetition is never evil, and a bounded repeat up to
      // SMALL_BOUNDED_REPEAT_MAX caps the ambiguity at
      // (branches)^m — `(a|a){2,4}` is at most 16 paths. Unbounded (`+`, `*`,
      // `{n,}`) or large bounded (`(a|a){2,50}` = 2^50) repeats stay flagged.
 ...
 *  ambiguity costs (choices)^m paths — a constant independent of input length
 *  — so a small m cannot freeze the engine ((a|a){2,4} ≤ 16 paths,
 *  (a{1,3}){2,4} ≤ 81) while a large one is astronomical ((a|a){2,50} = 2^50,
 *  which locks an engine at ~40 chars of input). 8 keeps the worst practical
 *  constant in the thousands. */
const SMALL_BOUNDED_REPEAT_MAX = 8

/** Parse the quantifier starting at position `i`: `+`, `*`, or a `{n}` /
 *  `{n,}` / `{n,m}` interval. Returns null when `i` starts no quantifier
 *  (including a `{` that is not a valid interval — that's a literal). The
 *  single quantifier grammar shared by hasNestedQuantifier and
 *  hasOverlappingAlternation, so the two guards can never disagree about
