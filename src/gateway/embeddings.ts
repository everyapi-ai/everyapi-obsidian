// Batch embeddings over the OpenAI-compatible `/v1/embeddings` endpoint. Mirrors chat.ts/http.ts: same auth headers, the same signal/timeout resolution, and a plain `fetch` POST with a JSON body — no SDK. Used by the VS Code assistant's opt-in codebase index (bulk-embed on an explicit user command) and by its search_codebase tool (one short query embed at search time).
...
export async function embed(input: EmbedInput): Promise<EmbedResult> {
  ...
  // Some gateways return HTTP 200 with a top-level `error` body instead of a non-2xx status (same pattern as chat.ts's completeChat/streamChat). Without this, `json.data` is absent and the code below would silently return an empty embeddings array as if the batch succeeded, instead of surfacing why nothing got embedded.
  if (json.error) {
    throw new Error(
      redactSecrets(
  ...
  // No data at all (an empty input batch or an absent `data` field) is a degenerate case with nothing to align — return empty rather than fabricate placeholder vectors.
  if (data.length === 0) {
    return { embeddings: [], usage: json.usage ?? undefined }
  }
  // Slot each embedding into its own `index` position so an out-of-order or sparse response can't shift vectors onto the wrong input.
  const rows = Array.from<number[]>({ length: input.input.length })
  let filled = 0
  // Some OpenAI-compatible gateways return the vectors in request order but omit the per-item `index`. When NOTHING carries an index and the count matches, positional order IS the alignment — use it rather than rejecting a response that's trivially safe to line up. Index-based slotting still kicks in the moment any item carries an index (guarding out-of-order / sparse).
  ...
  // A conforming endpoint returns exactly one vector per input; a dropped index would otherwise silently misalign every vector against its source. Count filled slots explicitly — `rows.some()` would skip the sparse-array holes.
