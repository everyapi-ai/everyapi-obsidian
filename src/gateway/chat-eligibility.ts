...
//
// RESIDUAL, not defended against here: a backend that confidently mis-seeds a real chat model with a RECOGNISED non-text token (`['image']` for gpt-4o) still hides it. Note the asymmetry that makes this the lesser problem: the vocabulary case is a client-side misreading of a backend that never claimed anything about `text`, this one is the backend making a claim and being wrong, and this project's standing rule is that capability is the backend's to state. There IS a second signal that could break the tie — `endpoint_kind` (chat / task), documented on the same payload at packages/api/src/endpoints.ts:124 — and it is deliberately unused rather than ruled out: nothing in this repo shows whether `/v1/models` (the endpoint both clients read, unlike the dashboard's `/api/models`) carries it, and inventing a dependency on an unverified field is the failure this module exists to avoid. Confirm it against the backend before reaching for it.
//
